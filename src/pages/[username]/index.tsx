...
import {ShareIcon, CalendarIcon, ChatAlt2Icon} from '@heroicons/react/outline';
...
import Services from 'components/PreLogin/Username/Services';
import dynamic from 'next/dynamic';
import ReactPlayer from 'react-player';
import HeadSeo from 'components/shared/HeadSeo/Seo';

const UserPage = ({user, credentialResponse}) => {
  ...
  const filteredServices = userServices?.filter(service => service.isPrivate === false);
                ...
                <div className='sm:col-span-2 text-sm'>
                  <dd
                    ...
                    dangerouslySetInnerHTML={{
                      __html: user?.personalDetails?.description,
                    }}
                  ></dd>
                </div>
              )}
            ...
            <div className='container mx-auto'>
              <div className='w-full rounded shadow-md'>
                {filteredServices.map((item, key) => (
                  <div className='bg-white  rounded xl:flex lg:flex w-full' key={key}>
                    <Services key={key} item={item} user={user} />
                  </div>
                ))}
              </div>
            </div>
...
export const getServerSideProps: GetServerSideProps = async context => {
  ...
  const credentialResponse: String[] = credentials?.data?.getCredentialsByUserId;
