...
import {ShareIcon, ChatAlt2Icon} from '@heroicons/react/outline';
import Calendar from 'components/PreLogin/Booking/Calendar';
...
const BookingPage = ({user, serviceData}) => {
  ...
  useEffect(() => {
    ...
    setUserPublicUrl(`${window.origin}/${window.location.href.split('/')[3]}`);
  }, [user]);
      ...
      <div className='absolute w-full h-full'>
        <div
          className={classNames([
            'my-6 lg:my-12 container px-6 mx-auto flex  lg:flex-row flex-col items-start lg:items-center justify-between',
        ...
        >
          <div className='w-full h-64 rounded '>
            <div className='bg-white  shadow rounded xl:flex lg:flex w-full'>
              <div
                className={classNames([
                  'w-full bg-gray-100 py-8 border-gray-300 xl:w-2/5 lg:w-2/5 xl:border-r rounded-tl xl:rounded-bl rounded-tr xl:rounded-tr-none lg:border-r-2 border-b xl:border-b-0  justify-center items-center',
                  serviceData?.serviceType === 'MEETING' ? '' : '',
                ])}
              >
                    ...
                    <div className='w-full'>
                      {user?.personalDetails?.description ? (
                        <div
                          ...
                          dangerouslySetInnerHTML={{__html: user?.personalDetails?.description}}
                        />
                      ) : (
                        ''
                      )}
              ...
              {serviceData?.serviceType === 'MEETING' ? (
                <div className='xl:w-3/5 lg:w-3/5 '>
                  <div className='px-3 py-2 text-sm w-full'>
                    <span className='font-bold text-4xl'>{serviceData?.title}</span>
                    ...
                    <div>
                      ...
                      [PAYMENT_TYPE.DIRECT, PAYMENT_TYPE.ESCROW].includes(
                  ...
                  <Calendar user={user} serviceData={serviceData} isAvailability={false} />
                </div>
              ) : (
                <div className='xl:w-3/5 lg:w-3/5'>
                  <div className='px-3 py-2 text-sm w-full'>
            ...
            {serviceData?.media?.link && serviceData?.media?.type === 'Google Slides' ? (
              serviceData?.media?.link.startsWith('https://docs.google.com/presentation') ? (
