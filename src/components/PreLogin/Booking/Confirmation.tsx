import {CalendarIcon, CheckIcon, ClockIcon} from '@heroicons/react/solid';
...
const Confirmation = ({user, serviceData, selectedDate, paymentInvoice}) => {
  ...
  const eventLink = () => {
    ...
    const event = ics.createEvent({
      start,
      startInputType: 'utc',
      title: `${serviceData.title} with ${user?.personalDetails?.name}`,
      description: serviceData.description,
      duration: {minutes: serviceData.duration},
    });
    ...
    <>
      <div className='grid  tracking-wide'>
        <div className='py-20 place-self-center items-center md:py-3'>
          <div className='grid grid-cols-1 justify-items-center px-10 py-10 bg-white rounded-b-lg shadow-2xl'>
            <div>
              ...
              <div className='mt-3 text-center sm:mt-5'>
                <h3 className='text-lg leading-6 font-medium text-gray-900' id='modal-headline'>
                  {t('meeting:book_confirm')}
                </h3>
                <div className='mt-2'>
                  <p className='text-sm text-gray-500'>
                    {t('meeting:you_sche_desc')} {user?.personalDetails?.name}.
                  </p>
                </div>
                <div className='mt-4 border-t border-b py-4'>
                  ...
                  <p className='text-gray-500'>
                    ...
                    {dayjs(selectedDate).format('hh:mma, dddd DD MMMM YYYY')}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className='mt-5 sm:mt-6 text-center'>
              ...
              <div className='flex justify-center mt-2'>
                <Link
                  href={encodeURI(
                    `https://calendar.google.com/calendar/render?action=TEMPLATE&dates=${dayjs(
                      selectedDate
                    ).format()}%2F${dayjs(selectedDate)
                      .add(serviceData.duration, 'minute')
                      .format()}&details=${serviceData.title} with ${
                ...
                <Link
                  href={encodeURI(
                    `https://outlook.live.com/calendar/0/deeplink/compose?body=${
                      serviceData.duration
                    }&enddt=${dayjs(selectedDate)
                      .add(serviceData.duration, 'minute')
                      .format()}&path=%2Fcalendar%2Faction%2Fcompose&rru=addevent&startdt=${dayjs(
                      selectedDate
                    ).format()}&subject=${serviceData.title} with ${user.accountDetails.username}`
                  )}
                >
                  <a className='mx-2 btn-wide btn-white'>
                    <svg
                ...
                <Link href={`data:text/calendar,${eventLink()}`}>
                  <a className='mx-2 btn-wide btn-white' download={`${serviceData.title}.ics`}>
                    <svg
                      version='1.1'
                      xmlns='http://www.w3.org/2000/svg'
              ...
              {paymentInvoice && (
                <div className='mt-5 sm:mt-6 text-center max-w-xl break-words overflow-hidden'>
                  <a
                    className=' text-gray-500 hover:text-mainBlue font-bold'
                    href={paymentInvoice}
                    target={'_blank'}
                    rel='noreferrer'
                  >
